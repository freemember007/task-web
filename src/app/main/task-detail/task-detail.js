  ...
  .controller('TaskDetailController', [
    ...
    function ($scope, $rootScope, Task, $timeout, LocalStorage, $filter) {
      ...
      $scope.$on('PleaseShowTaskDetail', function (event, msg) {
        ...
        $scope.task.hasImg = false;
        if ($scope.task.file && $scope.task.file.filename) {
          $scope.task.file.filename = $scope.task.file.filename.replace(/.*\\/, '');
          $scope.task.hasImg = $scope.task.file.filename.match(/(png)|(jpg)|(jpeg)|(gif)/); //todo: 保存时会把数据搞脏,待重构
        }
      ...
      $scope.alterTitle = function (e) {
        if(e.keyCode && e.keyCode != 13){
          return;
        }else if($scope.task.title && e.keyCode === 13){
          $(".taskNameInput").blur();
